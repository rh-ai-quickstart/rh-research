...
async def test_post_invoke_refuses_when_output_rail_fails(
    guardrails: _DeepAgentGuardrails,
    caplog: pytest.LogCaptureFixture,
    failure_point: str,
):
    """An output-rail runtime failure replaces deep-agent output with a refusal."""
    user_text = "Please summarize this issue."
    output_text = "Please follow up with customer@example.com about this issue."
    output = DeepResearchAgentState(messages=[HumanMessage(content=user_text), AIMessage(content=output_text)])
...
async def test_post_invoke_uses_schema_safe_emergency_refusal(
    ...
    """Traversal failure returns a minimal deep-agent state without protected output."""
    output_text = "Contact customer@example.com"
    log_sentinel = "traversal failed for jane.doe@example.com"
    output = DeepResearchAgentState(messages=[AIMessage(content=output_text)])
